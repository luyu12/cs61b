...
public class SList {
  ...
  private SListNode head;//成员属性
  private int size;//成员属性
  ...
  public boolean isEmpty() {
    return size == 0;//size==0是判断语句（T or F）等价于（if(size==0){return true;}）
  }
  ...
  public void insertFront(Object obj) {
    head = new SListNode(obj, head);//??put the obj before the head
    size++;
  }
  ...
  public void insertEnd(Object obj) {
    if (head == null) {//没有head节点（即空表？）
      head = new SListNode(obj);//obj为第一个node（即head）
    } else {//当有head节点时
      SListNode node = head;//设置新节点node which points to the head node
      while (node.next != null) {
    //从head后判断是不是空node。
        node = node.next;
    //如果是最后一个node，pointer后移一位
      }
      node.next = new SListNode(obj);//直到最后一个node时
      //跳出循环，在最后一个node后加上新node；
    }
    size++;//list的长度加1
  }
  ...
  public Object nth(int position) {
    ...
    if ((position < 1) || (head == null)) {
      ...
      currentNode = head;
      while (position > 1) {
        currentNode = currentNode.next;//pointer向后移一位
        if (currentNode == null) {//判断当前节点是否存在
          return null;
        }
        position--;
      }
      return currentNode.item;//获取【position】的节点里的数据
    }
  }
  ...
  public void squish() {
	  ...
	  SListNode fast = head;
	  while(fast != null) {
		  while(fast != null && fast.item.equals(slow.item)) {
			  fast = fast.next;
		  }
		  slow.next = fast;
		  slow = slow.next;
  ...
  public void twin() {
      ...
      while(node != null) {
    	  ...
    	  SListNode newNode = new SListNode(node.item);
    	  node.next = newNode;
    	  newNode.next = temp;
    	  node = node.next.next;
      }
